    ...
    return [
        Tool(
            ...
            description=(
                ...
                "【重要定義：用語の取り違え厳禁】\n"
                "1. rt (リアル実働時間): 実働時間から有休等の届出時間を差し引いた、現場での純粋な活動時間。★重要：賃金・給与計算とは一切無関係です。\n"
                "2. wt (実働時間): 休暇届出時間を含む集計値。契約労働時間(cw)との整合性が重要。\n"
                "3. ot (時間外): 残業時間のこと。cwとの差分。負の値は届出漏れの可能性を示唆します。\n"
                "【判定基準（仕様）】\n"
                "- 残業申請(oa)が'0'：原則として wt = cw となる仕様です。cw未満なら『イレギュラー』と判定します。\n"
                "- 残業申請(oa)が'1'：wt は実測値をそのまま返し、cw未満なら ot は負となります。\n"
                "- オンコール(oc)中：出勤(in)が00:00でも仕様上正常です。"
                "- 時間休(tr)：in・out にあらかじめ反映されている場合もあり、rtの算出のときに、二重に引かれている可能性があります。\n"
                ...
                PromptMessage(
                    ...
                    content=TextContent(
                        ...
                        text=(
                            ...
                            "2. リアル実働時間(rt)が、有休等の届出(am / pm)と矛盾なく算出されているか？\n"
                            "3. 時間休(tr)が申請されている日は、備考(rmk)をチェックすること。\n"
                            "4. 法的な適否ではなく、『システムの計算仕様として正しいか』を最優先に判断すること。\n"
